...
def add_to_manifest(repositories, fallback_branch = None, is_dependency = False):
    try:
        lm = ElementTree.parse(".repo/local_manifests/roomservice.xml")
        lm = lm.getroot()
    except:
...
def fetch_dependencies(repo_path, fallback_branch = None):
    ...
    if os.path.exists(dependencies_path):
        ...
        if len(fetch_list) > 0:
            ...
            add_to_manifest(fetch_list, fallback_branch,True)
...
if depsonly:
    ...
    if repo_path:
        ...
        print("Device tree not found on git using local tree")
